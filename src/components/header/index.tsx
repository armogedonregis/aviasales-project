...
const menuItems = [
    { label: "Billets d'avion", icon: "✈️" },
    { label: "Hôtels", icon: "🏨" },
    { label: "En bref", icon: "🕒" },
    { label: "Favoris", icon: "❤️" },
    { label: "Pour les entreprises", icon: "💼" },
];
...
export const Header = () => {
    ...
    return (
        <header className={`bg-blue-600 text-white transition-all duration-300 ${isScrolled ? 'fixed top-0 left-0 right-0 z-50 shadow-md' : ''}`}>
            <div className="container mx-auto px-4 py-2 flex lg:justify-between items-center">
                <LogoIcon />
                <div className="flex items-center ml-auto lg:ml-0 lg:mr-0 space-x-4">
                    <Link href="/profile" className="text-sm">
                        ...
                        <span className="lg:inline hidden">Profil</span>
                    </Link>
                    <Link href="/support" className="text-sm">
                        ...
                        <span className="lg:inline hidden">Support</span>
                    </Link>
                    <div className="flex items-center">
                        ...
                        <select className="bg-blue-600 text-sm lg:block hidden">
                            <option>EUR</option>
                        </select>
                    </div>
                </div>
                <div className="flex items-center ml-5 lg:ml-0 space-x-4 md:hidden">
            ...
            {!isScrolled && (
                <div className="container mx-auto px-4 py-8">
                    <h1 className="text-2xl md:text-4xl font-bold mb-6 text-center">Ici, on achète des billets d&lsquo;avion pas chers</h1>
                    <nav className="mb-6 flex justify-center">
                        <ul ref={menuRef} className="flex bg-blue-700 rounded-full relative p-1 overflow-x-auto md:overflow-x-visible">
                            <div
                            ...
                            {menuItems.map((item, index) => (
                                <li key={index} className="relative z-10 flex-shrink-0">
                                    <button
                                        className={`px-3 md:px-4 py-2 rounded-full flex flex-col items-center ${index === activeIndex ? "text-blue-600" : "text-white"}`}
                        ...
                        <div className="flex-1">
                            ...
                            {showDatePicker && (
                                <div className="absolute z-20 mt-2">
                                    <DatePicker onSelectDate={handleDateSelect} initialDate={new Date(2024, 8, 6)} onClose={() => setShowDatePicker(false)} />
                                </div>
                            )}
                        </div>
                        <div className="flex-1">
                            ...
                            {showDatePickerTwo && (
                                <div className="absolute z-20 mt-2">
                                    <DatePicker onSelectDate={handleDateSelectTwo} isReturnDate initialDate={new Date(2024, 8, 6)} onClose={() => setShowDatePickerTwo(false)} />
                                </div>
                            )}
                        </div>
                        <div className="flex-1">
                    ...
                    <div className="mt-4 text-sm flex flex-col md:flex-row justify-between items-center">
                        <button className="text-white underline mb-2 md:mb-0">Créer un itinéraire complexe</button>
                        <label className="flex items-center">
                            <input type="checkbox" className="mr-2" />
                            <span>Ouvrir Ostrovok.ru dans un nouvel onglet</span>
                        </label>
                    </div>
                </div>
            )}
