        ...
        <div className="space-y-4 mb-4">
             {step === 1 && (selectedInitialSuggestion === t('plan3DayUAE') || selectedInitialSuggestion === t('weekendAmsterdam')) && (
                <div className="relative pt-10">
                    <div className="absolute top-0 right-0 z-10 space-x-2">
                        <button onClick={() => sliderRef.current?.slickPrev()} className="bg-white text-blue_primary rounded-full p-2 shadow-md">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                            </svg>
                        </button>
                        <button onClick={() => sliderRef.current?.slickNext()} className="bg-white text-blue_primary rounded-full p-2 shadow-md">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
                                ...
                                <div className="bg-white rounded-lg shadow-md p-2 flex flex-col h-[130px] lg:h-[200px]">
                                    {index === 0 ? (
                                        <div className="w-full h-14 lg:h-[130px] flex items-center justify-center bg-blue-100 rounded-t-lg flex-shrink-0">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-blue_primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className={`flex ${step === 0 ? 'flex-col items-end space-y-2' : 'flex-row space-x-2'}`}>
                {currentSuggestions.map((suggestion, index) => (
                    <button
                        key={index}
                        className={`bg-blue_primary text-white rounded-full px-4 py-2 text-sm hover:bg-blue-600 transition-colors duration-200 ${step === 0 ? 'text-left' : ''}`}
